...
tBST LeftChild(tBST tree) { return tree->left; } //DONE

tBST RightChild(tBST tree) { return tree->right; } //DONE

int max(int a, int b){ //Returns the maximum of two integers //DONE
	if (a>=b) {
		return a;
	}else {
		return b;
...
int height(tBST tree){ //Returns tree height //DONE
...
void rr_rotation(tBST *tree){ //left rotate //DONE
    if (tree==NULLBST) { return;}
...
void ll_rotation(tBST *tree){ //right rotate //DONE
    if (tree==NULLBST) { return;}
...
void rl_rotation(tBST *tree){ //DONE
    if (tree==NULLBST) { return;}
...
void lr_rotation(tBST *tree){ //DONE
    if (tree==NULLBST) { return;}
...
bool createBSTNode (tBSTPos* p,tKey key){  //DONE
...
bool insertKey(tBST* tree, tKey key)  //DONE
{
    bool check = false;
...
void replace (tBST* subTree,tBST* auxTree){  //Replace the content of a node by its predecessors //Works
...
void removeKey(tBST* tree,tKey key){ //Changes //TODO
    ...
    if (key < (*tree)->key) { //== 
        ...
        removeKey(&(*tree)->right, key);    //If the key is larger, continue through right subtree   
          ...
          printf("Removing: %d\n",key);                           //If it has two children we call replace() passing 
          aux = *tree;
          ...
          { //If it has no left child, replace by right one.
            ...
            printf("Removing: %d\n",key);                           //If it has two children we call replace() passing 
            aux = *tree;                                 //its left subtree as first argument
    ...
    //HERE. Falta configurar que colla como aux2 na rotacion o nodo con mais
    //altura e en caso de empate o que sea simple
